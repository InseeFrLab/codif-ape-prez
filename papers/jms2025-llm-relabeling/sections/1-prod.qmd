...
C'est dans ce contexte que Sirene et ses gestionnaires ont dû être en mesure de gérer un flux constant de libellés d'activité à codifier. Pour cela, ils ont mis en place une méthodologie adaptée pour gérer à la fois le volume, la fréquence et la variété des descriptions saisies, qui se comptent dans l'ordre du million par an, ainsi que des libellés pouvant être déroutants.
...
Ce répertoire de référence enregistre l'état civil de toutes les entreprises et de leurs établissements et gère leur cycle complet: de leur naissance à leur cessation. C'est un processus vivant qui continue à traiter les différents évènements du vivant de l'entreprise.
...
En tant que répertoire de référence, les exigences de qualité du code APE, la disponibilité de l'application et la nécessité d'un traitement régulier des formalités sont inhérentes à sa mission historique. Ce rôle exige toujours la mobilisation constante des gestionnaires; ce n'est donc pas la nature du travail qui change, mais son unité.
...
Ce nouveau partenariat exige de l'Insee qu'elle soit capable de répondre rapidement. Le traitement en flux continu impose une **gestion unitaire et très réactive des dossiers d'entreprises**, avec la flexibilité nécessaire pour les débloquer. C'est une performance que le traitement par lots (batch) peut difficilement garantir, ce dernier nécessitant d'attendre la prochaine transmission.
...
Au cœur de cette architecture réside un workflow de traitement structuré. Ce dernier séquence de manière souple les différentes étapes d'analyse et de codification, assurant la flexibilité nécessaire à la gestion unitaire des dossiers. En somme, le traitement des tâches répétitives est automatisé, ce qui a deux effets majeurs : un soulagement de la charge sur le circuit et une concentration de l'activité humaine sur les cas complexes à forte plus-value. Ce transfert d'expertise pose de nouveaux défis structurels et d'organisation.

Bien que l'application Sirene se soit modernisée, l'outil Sicore ne convenait plus face au **format non-structuré des libellés provenant du Guichet Unique**. Son taux de codage automatique n'atteignait que 30%, impliquant près de 70% de reprise manuelle et surchargeant considérablement les équipes.

De plus, le fonctionnement du système expert Sicore était fortement adhérent à l'ancienne architecture de l'application de gestion du répertoire (Sirene 3). La nouvelle architecture en micro-services de Sirene 4 a permis de découpler cet outil, rendant le système de classification moins dépendant de l'application principale. Il a ainsi été facilité de rapidement le remplacer par un modèle d'apprentissage supervisé, dès le début du projet, tirant pleinement parti de la modularité de Sirene 4.
...
Le _seuil_ théorique recommandé se situe entre 0.65 et 0.7. Par précaution, Sirene 4 avait initié un _seuil_ de 0,8 qui a depuis été abaissé plus proche du _seuil_ recommandé pour soulager la charge.
...
*Approche méthodologique partagée par d'autres projets de codification automatique à l'Insee*
:::
...
Bien que la codification automatique soit la plus prépondérant[^odg-codif], le rôle du gestionnaire est crucial pour traiter les cas incertains et garantir la qualité.
...
::: {.center}
![Exemple de poste de reprise de la codification](img/reprise_gestionnaire_manque_contexte.png){#fig-poste-reprise-gestionnaire width=70%}
...
Un modèle adéquat pour Sirene 4 ne se limite pas à atteindre l'état de l'art, aussi appelé State of the Art ou SOTA. L'adoption en environnement de production engage plusieurs facteurs critiques, cités à titre d'exemple :
...
*Architecture simplifié avec plongement lexical et moyennage des plongements (ou des vectorisations), classification des vecteurs, puis conversion des scores en probabilité.*
:::
...
- performance et coût : Le modèle doit maintenir un temps d'inférence acceptable (inférieure à 20 ms) et un taux d'automatisation élevé. Le nouveau modèle est conçu pour être plus léger, facilitant le transfert rapide et préparant le système à une inférence GPU pour une évolutivité verticale future (même si les GPU ne sont pas encore en production).
...
Actuellement, la librairie _fastText_ est toujours utilisée. Toutefois, le passage à la nouvelle librairie est programmé pour cette fin d'année 2025, en même temps qu'une migration de la codification probabiliste sur Kube.
...
::: {.center}
![Environnement et gestion de l'entraînement du modèle ](img/api-datalab.png){#fig-model-store width=70%}
...
L'enjeu fondamental est d'intégrer la surveillance dans le flux de travail habituel (tâche routinière). L'équipe devra reprendre cette surveillance avec la NAF 2025 dès son déploiement pour piloter le nouveau modèle, garantissant une qualité constante des données du répertoire.

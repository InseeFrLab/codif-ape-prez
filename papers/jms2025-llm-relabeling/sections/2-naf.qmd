...
Les correspondances **univoques** (ou bijections) désignent les cas où un code de l'ancienne nomenclature (NAF rev 2) correspond à un seul code dans la nouvelle nomenclature (NAF 2025). Ces cas ne présentent pas de difficulté majeure pour la conversion automatique des données d'apprentissage.

::: {.center}
![Exemple de correspondance univoque 1:1](img/univoques.png){#fig-bijection}
...
: Distribution de codes univoques ou bijections (Cette table indique qu'une large majorité des codes, 75%, sont univoques.) {#tbl-dist-univoques}
:::
...
::: {.center}
![Exemple de correspondance N:1 ou surjection](img/surjection.png){#fig-surjection}
...
::: {.center}
![Exemple de correspondance multivoque 1:N](img/multivoques.png){#fig-multivoques}

*Cette figure illustre un éclatement (1:N) où un ancien code correspond à plusieurs codes possibles de la NAF 2025.*
:::

Les **multivoques désigneront**, tout au long de cet article, principalement les correspondances 1:N (éclatements), où N représente le nombre de possibilités dans la nouvelle nomenclature. Ces cas, qui concernent 25 % des codes, sont ceux qui requièrent une annotation manuelle pour déterminer la
...
Cette table de passage métier identifie les "faux" multivoques[^remarque-tbl-passage]. Par exemple, le code 6820A ("Location de logements") est souvent requalifié en 6820G dans 16% des cas, en partant du postulat métier que ces locations sont majoritairement de longue durée. De même, les activités de production d'électricité des personnes physiques (3511Z) sont approximées comme de l'électricité de type renouvelable (3512Y).
...
Grâce à cette approche métier, 32,5 %, soit près d'un tiers, des codes multivoques initialement identifiés ont pu être corrigés ou reclassifiés dans la base d'apprentissage. Cependant, cette méthode ne peut raisonnablement s'appliquer qu'à moins de 50% des multivoques dans le jeu, dont le volume total (univoques et multivoques) atteint deux millions d'observations. Même en reclassifiant la moitié de ces cas, cela laisserait environ 500 000 multivoques à convertir, démontrant que l'efficacité de cette méthode est insuffisante pour éviter une campagne d'annotation massive.
...
- 0: aucune difficulté ou rien à signaler 
- 1: plutôt facile [^default-value]
- 2: facile 
- 3: moyen
- 4: difficiles 
- 5: très difficiles

[^default-value]: La modalité 1 (plutôt facile) correspond à la valeur par défaut de la plateforme. Par conséquent, cette modalité est peu considérée et n'a pas été prise en compte dans les résultats affichés dans les graphes ultérieurs.
